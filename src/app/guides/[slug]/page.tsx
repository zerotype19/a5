...
import { loadGuidePage } from "@/lib/authority/engine";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const result = await loadGuidePage(slug);
  if (result.status !== "ok") {
    return { title: "Not found", robots: { index: false, follow: false } };
  }
  const path = buildContentPathFromRecord(result.page) ?? `/guides/${slug}`;
  return buildContentMetadata({ page: result.page, path });
}

export default async function GuideAuthorityPage({ params }: Props) {
  const { slug } = await params;
  const result = await loadGuidePage(slug);
